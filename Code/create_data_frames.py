...
def create_data_frames(isin_list):
    ...
    financials_df = pd.concat(financial_data, ignore_index=True, keys=df['Company name'])[
        [
            'Company name', 'asOfDate', 'periodType', 'currencyCode',
            'AccountsPayable', 'AccountsReceivable',
            'AccumulatedDepreciation', 'AvailableForSaleSecurities',
            'BasicAverageShares', 'BasicEPS',
            'BeginningCashPosition', 'TotalDebt', 'TotalEquityGrossMinorityInterest',
            'TotalExpenses',
...
if __name__ == "__main__":
    combined_data, profile_df, financials_df = create_data_frames([])
    
    # Create a Pandas Excel writer using ExcelWriter
    excel_filename = 'Data/Data_Frames/combined_esg_data.xlsx'
    with pd.ExcelWriter(excel_filename) as writer:
        ...
        financials_df.to_excel(writer, sheet_name='Financials', index=True)
    ...
    output_excel_filename = 'Data/Data_Frames/latest_financials.xlsx'
